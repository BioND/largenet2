...
unsigned long int CMyRng::getSeed() const
{
	return seed_;
}
...
void CMyRng::TestUniform01()
{
	...
	for (i = 0; i < 10; i++)
	{
		printf("    %1.1lf...%1.1lf: %1.1lf%% [10.0%%]\n", (double) (i) / 10.0,
				(double) (i + 1) / 10.0, ((double) x[i] / 1e7) * 100.0);
	}
	printf("    Time: %lds\n", t2 - t1);
	printf("\n");
}
...
void CMyRng::TestFromTo()
{
	...
	for (i = 0; i < 10; i++)
	{
		printf("    %+1.3lf...%+1.3lf: %1.1lf%% [10.0%%]\n", from
				+ (double) (i) * (to - from) / 10.0, from + (double) (i + 1)
				* (to - from) / 10.0, ((double) x[i] / 1e7) * 100.0);
	}
	printf("    Time: %lds\n", t2 - t1);
	printf("\n");
}
...
void CMyRng::TestIntFromTo()
{
	...
	for (i = 0; i < (to - from) + 1; i++)
	{
		printf("    %2d: %2.1lf%% [%2.1lf%%]\n", i + from,
				((double) x[i] / 1e7) * 100.0, 100.0 / (double) (to - from + 1));
	}
	printf("    Time: %lds\n", t2 - t1);
	printf("\n");
}
...
void CMyRng::TestChance()
{
	...
	for (i = 0; i < 10; i++)
	{
		printf("    Chance %2.1lf: Success %2.1lf%% [%2.1lf%%]\n", 100.0
				* chance[i], ((double) x[i] / 1e6) * 100.0, 100.0 * chance[i]);
	}
	printf("    Time: %lds\n", t2 - t1);
	printf("\n");
}
...
void CMyRng::TestChoices()
{
	...
	for (i = 0; i < numchoice; i++)
	{
		printf("    Choice %d (%3.1lf%%): %3.1lf%% [%3.1lf%%]\n", i, 100.0
				* chance[i], ((double) x[i] / 1e7) * 100.0, 100.0 * chance[i]);
	}
	printf("    Time: %lds\n", t2 - t1);
	printf("\n");
}
...
void CMyRng::TestNormal01Polar()
{
	...
	for (i = 1; i < 10; i++)
	{
		printf("    %+1.1lf...%+1.1lf: %2.1lf%% [%2.1lf%%]\n", (double) (i - 5)
				/ 2.0 - 0.5, (double) (i - 5) / 2.0 + 0.5, ((double) x[i]
				/ testset) * 100.0, erf[i]);
	}
...
void CMyRng::TestGamma()
{
	...
	for (int j = 0; j < 2; j++)
	{
		...
		if (j == 0)
			printf(
					"    Test task: Mean %2.2lf, Variance %2.2lf for shape < 1 (shape %2.2lf scale %2.2lf)\n",
...
void CMyRng::TestBeta()
{
	...
	double targetvar = (shape1 * shape2) / ((shape1 + shape2) * (shape1
			+ shape2) * (shape1 + shape2 + 1.));
	printf(
			"    Test task: Mean %2.2lf Variance %2.2lf (shape1 %2.2lf shape2 %2.2lf)\n",
 ...
 */
{
	...
	//printf("\n");
	// This could be a really bad initialization, so
	// lets "escape from zerloand".
...
double CMyRng::BaseRand()
{
	...
	z2 = (STATE[(state_i + 24) & 0x0000001fU] ^ (STATE[(state_i + 24)
			& 0x0000001fU] << (19))) ^ (STATE[(state_i + 10) & 0x0000001fU]
			^ (STATE[(state_i + 10) & 0x0000001fU] << (14)));

	STATE[state_i] = z1 ^ z2;
	STATE[(state_i + 31) & 0x0000001fU] = (z0 ^ (z0 << (11))) ^ (z1 ^ (z1
			<< (7))) ^ (z2 ^ (z2 << (13)));
...
CMyRng rng; // This is my WELL rng
